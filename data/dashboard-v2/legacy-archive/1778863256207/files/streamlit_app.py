...
def main() -> None:
    ...
    st.caption("DASHBOARD REFACTOR - NO LIVE TRADING")
...
def _render_performance() -> None:
    st.subheader("Performance & Resource Budgets")
    st.caption("LOCAL PERFORMANCE PROFILING ONLY - NO LIVE TRADING")
    from binance_spot_bot.dashboard_profiler import profile_dashboard_panels
    from binance_spot_bot.performance_budget import evaluate_performance_budget
    ...
    dashboard_profile = profile_dashboard_panels(["overview", "demo_spot_trading", "test_selection", "performance"])
    budget = evaluate_performance_budget("dashboard_panel_ms", dashboard_profile["summary"]["max_duration_ms"])
    regression = detect_performance_regression("dashboard_panel_ms", 10, dashboard_profile["summary"]["max_duration_ms"], 750)
    resources = resource_snapshot(Path.cwd())
    render_badges(
        {
            "Budget": budget["status"],
            "Legacy": payload["status"],
            "Max ms": dashboard_profile["summary"]["max_duration_ms"],
            "Regression": regression["status"],
            "Data MB": round(resources["data_dir_size_bytes"] / 1024 / 1024, 2),
            "Live trading": "disabled",
        }
    )
    render_table("Slowest spans", dashboard_profile["summary"]["slowest_spans"])
    with st.expander("Performance recommendations"):
        st.json(performance_recommendations(dashboard_profile["summary"]))
    with st.expander("Resource snapshot"):
        st.json(resources)
...
def _render_paper_os_audit() -> None:
    ...
    st.caption("PAPER OS MILESTONE - NO LIVE TRADING")
    ...
    from binance_spot_bot.system_inventory import system_inventory
    from binance_spot_bot.system_safety_invariants import system_safety_invariants

    payload = paper_os_simulation()
    proof = no_live_proof_pack()
    readiness = production_readiness_simulation()
    inventory = system_inventory()
    invariants = system_safety_invariants()
    render_badges(
        {
            "Status": payload["payload"]["status"],
            "Readiness grade": readiness["score"]["grade"],
            "Subsystems": len(inventory["subsystems"]),
            "Invariants": len(invariants["invariants"]),
            "No-live proof": proof["status"],
            "Live trading": "disabled",
        }
    )
    render_table(
        "Subsystem readiness",
        [
            {"subsystem": row["name"], "status": row["status"], "score": row["readiness_score"]}
            for row in inventory["subsystems"][:10]
        ],
    )
...
def _render_stabilization() -> None:
    ...
    st.caption("PAPER OS STABILIZATION - NO LIVE TRADING")
    ...
    from binance_spot_bot.flaky_check_burndown import flaky_check_burndown
    ...
    backlog = build_stabilization_backlog([])
    gate = evaluate_stabilization_gate(backlog)
    reliability = check_reliability([{"name": "dashboard", "status": "ok", "duration_ms": 100}])
    flaky = flaky_check_burndown([])
    slow = detect_slow_checks([{"name": "dashboard", "duration_ms": 100}])
    gaps = detect_evidence_gaps(["no_live_proof"], ["no_live_proof"])
    render_badges(
        {
            "Gate": gate["status"],
    ...
    render_table("Open blockers", backlog["items"])


def _render_operator_training() -> None:
    st.subheader("Operator Training")
    st.caption("OPERATOR TRAINING - NO LIVE TRADING")
    from binance_spot_bot.dashboard_walkthroughs import dashboard_walkthroughs
    from binance_spot_bot.no_live_training import no_live_training_lesson
    from binance_spot_bot.operator_certification import certification_draft
    ...
    from binance_spot_bot.training_scenarios import training_scenarios
    ...
    walkthroughs = dashboard_walkthroughs()
    scenarios = training_scenarios()
    playbooks = troubleshooting_playbooks()
    lesson = no_live_training_lesson()
    cert = certification_draft()
    render_badges(
        {
            "Lessons": len(lessons["lessons"]),
            "Commands": len(cookbook["commands"]),
            "Walkthroughs": len(walkthroughs["walkthroughs"]),
            "Scenarios": len(scenarios["scenarios"]),
            "Playbooks": len(playbooks["playbooks"]),
            "No-live": lesson["status"],
            "Certification": cert["status"],
            "Live trading": "disabled",
        }
    )
    render_table("Glossary", [{"term": key, "meaning": value} for key, value in glossary["terms"].items()])
