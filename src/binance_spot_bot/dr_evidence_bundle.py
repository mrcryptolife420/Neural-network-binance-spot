...
def export_dr_evidence_bundle(root: Path, files: list[Path]) -> dict[str, Any]:
    bundle_id = f"dr-evidence-{int(time.time() * 1000)}"
    out = Path(root) / "disaster-recovery" / "evidence" / bundle_id
    files_dir = out / "files"
