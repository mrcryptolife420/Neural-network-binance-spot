...
Gebouwd: Production local packaging, desktop shortcut, rollback and offline recovery kit.
...
* Nieuwe `packaging` package met package profiles, dependency lock, build manifest, portable bundle, Windows installer scripts, shortcuts, startup health, update guard, migration preview, backup/restore, rollback, offline recovery kit, safe mode, package verification, package evidence en pipeline.
* Packaging safety/docs onder `docs/packaging/`.
* CLI-oppervlak voor package profiles, lock, build manifest, portable build, installer build, shortcuts, startup health, update plan, migration preview, backup, restore preview, rollback preview, recovery kit, safe mode, verify, evidence export en Dashboard V2 package smoke.
* Dashboard V2 Package Center op `/package`.
* API-routes onder `/api/package/*`.
* Check-all entries voor package profiles, update plan en Dashboard V2 package smoke.
* Acceptatietests in `tests/test_roadmap_122_packaging_acceptance.py`.

Validatie:

* `python -m compileall -q src tests`
* `pytest -q tests/test_roadmap_122_packaging_acceptance.py` -> 4 passed
* CLI smokes voor `package-profiles`, `package-update-plan`, `package-recovery-kit-build`, `dashboard-v2-package-smoke`
...
* Playwright screenshot smoke voor `/package`

Safety:

* Installer, updater, rollback en recovery kit starten live trading nooit automatisch.
* Safe defaults blijven `LIVE_TRADING_ENABLED=false` en `KILL_SWITCH=true`.
* Update guard blokkeert bij active live session.
* Restore/rollback forceert live locked en invalideert live arm-token conceptueel.
* Package evidence bevat no-live-auto-start en no-secret proofs.
