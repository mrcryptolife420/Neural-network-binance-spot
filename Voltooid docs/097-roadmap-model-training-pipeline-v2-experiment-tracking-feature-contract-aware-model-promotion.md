...
Gebouwd:
- training job config schema met data binding, model spec, split/cost/risk/validation policies;
- local experiment tracker met redacted run index;
- echte training pipeline bovenop `TinyNeuralSignalModel` en synthetic deterministic training rows;
- model artifact manifest met sha256, feature schema hash, dataset id en metrics;
- inference compatibility check en latency budget;
- paper/shadow/demo-only promotion gate v2;
- model card v2 en redacted model evidence bundle;
- docs voor safety contract, config, tracker, artifact manifest, promotion gate en evidence bundle.

Validatie:
- `python -m pytest -q` -> 362 passed, 1 warning;
- `python -m pytest tests/test_roadmap_097_model_training_acceptance.py tests/test_roadmaps_097_102_full_surface.py::test_097_training_pipeline_and_promotion_gate_are_contract_gated -q` -> groen;
- modelops/governance regressie -> 21 passed;
- `python -m binance_spot_bot.cli check-all --skip-tests --json` -> ok;
- `python -m binance_spot_bot.cli dashboard-smoke --seconds 1` -> ok.

Safety:
- promotion scope is alleen `paper_shadow_demo_only`;
- operator confirmation blijft vereist;
- artifacts/evidence worden geredact;
