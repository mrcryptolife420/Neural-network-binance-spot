...
Gebouwd:
- model monitoring config met scope, drift/performance thresholds, downgrade policy en schedule policy;
- feature, prediction en confidence drift checks;
- paper performance monitor voor PnL/drawdown degradatie;
- model health score met drift/performance/latency/calibration componenten;
- paper/shadow/demo-only downgrade policy en executor met alias allowlist/blocklist;
- shadow prediction JSONL logger met redaction;
- model monitoring pipeline die drift, performance, health en downgrade recommendation combineert;
- safety/config/shadow logging/drift downgrade docs.

Validatie:
- `python -m pytest -q` -> 367 passed, 1 warning;
- `python -m pytest tests/test_roadmap_098_model_monitoring_acceptance.py tests/test_roadmaps_097_102_full_surface.py::test_098_monitoring_detects_drift_and_requires_confirmed_downgrade -q` -> groen;
- brede 097-102/paper-os regressie -> 20 passed;
- `python -m binance_spot_bot.cli check-all --skip-tests --json` -> ok;
- `python -m binance_spot_bot.cli dashboard-smoke --seconds 1` -> ok.

Safety:
- downgrade blokkeert `champion_live`, `live_approved` en `auto_live`;
- downgrade vereist `DOWNGRADE_PAPER_MODEL` en evidence bij history writes;
- monitoring plaatst geen orders en gebruikt geen signed/account endpoints;
- live trading blijft disabled.
