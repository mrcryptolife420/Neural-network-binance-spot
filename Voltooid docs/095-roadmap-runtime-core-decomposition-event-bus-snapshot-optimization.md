...
Gebouwd:
- typed `RuntimeEvent` en backward-compatible `RuntimeEventBus`;
- expliciete `RuntimeState` dataclasses voor identity, lifecycle, market, paper, model, demo, reports en safety;
- snapshot builder met `compact`, `dashboard`, `full` en `evidence` profiles;
- recursieve snapshot limits en redaction;
- runtime step pipeline met stage events en stop-on-failure gedrag;
- batched local JSONL session/event writer;
- demo pilot service boundary voor counters/status;
- runtime refactor safety docs, state model docs, event bus docs, snapshot profile docs en step pipeline docs.

Validatie:
- `python -m pytest -q` -> 354 passed, 1 warning;
- `python -m pytest tests/test_roadmap_095_runtime_core_acceptance.py tests/test_roadmaps_089_096_full_surface.py::test_095_runtime_event_bus_and_snapshot_limits -q` -> groen;
- brede 089-096 regressie -> 31 passed;
- `python -m binance_spot_bot.cli check-all --skip-tests --json` -> ok;
- `python -m binance_spot_bot.cli dashboard-smoke --seconds 1` -> ok.

Safety:
- event bus voert geen trading side effects uit;
- snapshot payloads zijn redacted en gelimiteerd;
- batch writer schrijft alleen lokale JSONL artifacts;
- live trading blijft disabled en er zijn geen signed/account/order endpoints toegevoegd.
