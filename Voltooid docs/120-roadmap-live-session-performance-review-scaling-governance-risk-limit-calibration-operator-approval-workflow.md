...
* Live governance safety constants met expliciete no-auto-scale en operator approval bevestiging.
* Live session review, scorecards, execution-quality analytics, risk-limit calibration, scaling-governance decisioning, operator approval workflow, profile lifecycle, risk-preset proposals, session regression en governance evidence export.
* CLI-oppervlak voor live governance status, review, scorecard, execution quality, calibration, scaling decision, approval request/decision, profile lifecycle, risk preset proposal, regression, evidence export en Dashboard V2 smoke.
* Dashboard V2 Live Governance Cockpit op `/live/governance`.
* API-routes onder `/api/live-governance/*` voor read-only governance acties en approval-gated decisions.
* Docs onder `docs/live-trading/` voor safety contract, review, scorecards, calibration, approvals, lifecycle, regression, evidence en cockpit.
* Check-all integratie en acceptatietests in `tests/test_roadmap_120_live_governance_acceptance.py`.

Validatie:

* `python -m compileall -q src tests`
* `pytest -q tests/test_roadmap_120_live_governance_acceptance.py` -> 4 passed
* CLI smokes voor `live-session-review`, `live-session-scorecard`, `live-approval-decide` blocked/approved flows
...
* `python -m binance_spot_bot.cli dashboard-v2-live-governance-smoke --json`
* `python -m binance_spot_bot.cli security-scan` -> geen findings
* Playwright screenshot smoke voor `/live/governance`
* `pytest -q` -> 463 passed, 1 warning

Safety:

* Governance plaatst geen orders en start geen live sessies.
* Scale-up blijft geblokkeerd zonder exacte operator approval.
* Risk-increase en profile-promotion zijn proposal/approval-gated.
* Reports en evidence blijven secret-free volgens security scan en redaction checks.
