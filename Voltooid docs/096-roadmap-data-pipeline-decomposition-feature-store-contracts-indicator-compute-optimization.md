...
Gebouwd:
- `DataStoreRoot`, path policy, artifact refs en manifests met hashes;
- data quality v2 met ordering, duplicate en gap checks;
- indicator registry met SMA, volatility en momentum;
- feature store contracten met schema hash validatie;
- incremental feature builder op basis van bestaande deterministic `build_feature_rows`;
- data pipeline lineage en redacted evidence export;
- data pipeline safety contract, data store contracts, feature contracts, incremental compute en lineage docs.

Validatie:
- `python -m pytest -q` -> 357 passed, 1 warning;
- `python -m pytest tests/test_roadmap_096_data_pipeline_acceptance.py tests/test_roadmaps_089_096_full_surface.py::test_096_data_pipeline_contracts_indicator_compute -q` -> groen;
- brede 089-096 regressie -> 34 passed;
- `python -m binance_spot_bot.cli check-all --skip-tests --json` -> ok;
- `python -m binance_spot_bot.cli dashboard-smoke --seconds 1` -> ok.

Safety:
- pipeline is local/public-data only;
- manifests en evidence worden geredact;
- path traversal wordt geblokkeerd;
- geen signed/account/order endpoints toegevoegd;
- live trading blijft disabled.
