...
* Nieuwe `ai_doctor` package met run schema, start/finish snapshots, observer events, error/log collectors, system-state collector, check-all capture, codebase context, known issue matcher, root-cause hypotheses, AI summary writer, Codex prompt writer, debug pack builder, evidence bundle, Codex task classifier, repair suggestions, AI context pack en pipeline.
* AI Doctor safety/docs onder `docs/ai-doctor/`.
* CLI-oppervlak voor start, event, finish, crash report, status, collect, match issues, summary, Codex prompt, export, evidence export, verify en Dashboard V2 smoke.
* Dashboard V2 AI Doctor Center op `/ai-doctor`.
* API-routes onder `/api/ai-doctor/*`.
* Check-all entries voor AI Doctor status, export en Dashboard V2 AI Doctor smoke.
* Acceptatietests in `tests/test_roadmap_123_ai_doctor_acceptance.py`.

Validatie:

* `python -m compileall -q src tests`
* `pytest -q tests/test_roadmap_123_ai_doctor_acceptance.py` -> 5 passed
* CLI smokes voor `ai-doctor-status`, `ai-doctor-export`, `dashboard-v2-ai-doctor-smoke`
...
* Playwright screenshot smoke voor `/ai-doctor`

Safety:

* AI Doctor is local-only/read-only voor debug en evidence.
* AI Doctor plaatst geen orders, start geen live sessies en voert geen recovery automatisch uit.
* Debug bundle en evidence bevatten no-live/no-order/local-only proofs.
* Codex prompt bevat expliciete no-live/no-secret constraints.
* Error/log collectors redacteren secret-like payloads.
