...
Gebouwd: demo-to-live live-training pipeline met session targets, demo recorder/vault, dataset quality burn-down, quality gate v2, feature/label dataset, split governance, model candidate registry, model/strategy validation, paper replay, testnet promotion gate, testnet rehearsal confirm gate, live candidate blocker gate, Dashboard V2 `/live-training`, CLI commands, docs en check-all integratie.

Validatie: `python -m compileall -q src tests`, `pytest -q tests/test_roadmap_117_demo_to_live_training_acceptance.py`, 117 CLI smokes, Vite build, Dashboard V2 static verify, Playwright static browser smoke voor `/live-training`, `python -m binance_spot_bot.cli security-scan`, `python -m binance_spot_bot.check_all --skip-tests`, en volledige `pytest -q`.

Resultaat volledige test-suite: 451 passed, 1 warning.

Safety: live trading en live execution blijven uitgeschakeld; testnet rehearsal vereist `RUN_TESTNET_REHEARSAL_ONLY`; live candidate gate blijft geblokkeerd tot een aparte live execution implementation gate.
