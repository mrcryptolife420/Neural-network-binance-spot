...
```text
Voltooid docs/077-roadmap-data-driven-strategy-confidence-backtest-dataset-builder-indicator-calibration.md
```
...
- strategy_calibration.py toegevoegd voor dataset builder, no-lookahead guard, chronological train/validation/test splits, confidence calibration, symbol ranking, indicator profile ranking, backtest calibration en paper-only promotion gate.
- Strategy calibration reports toegevoegd: JSON latest, markdown, symbol ranking CSV en indicator ranking CSV.
- CLI command toegevoegd: strategy-calibrate met JSON output.
- Tests toegevoegd: 	ests/test_roadmap_077_strategy_calibration.py.

Validatie:

- python -m pytest tests/test_roadmap_076_public_data_ingestion.py tests/test_roadmap_077_strategy_calibration.py tests/test_features_model_backtest.py tests/test_indicators.py -q -> 15 passed.
