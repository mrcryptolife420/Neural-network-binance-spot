...
Gebouwd: live-safety package met evidence prerequisite gate, read-only account verifier, endpoint policy, live dry-run session, order preview, sizing guard, arm-token flow, kill-switch/cancel drills, first-order fake-adapter gate, live execution adapter, session state machine, audit hash chain, evidence bundle, Dashboard V2 `/live`, live API routes, CLI commands, docs en check-all integratie.

Validatie: `python -m compileall -q src tests`, `pytest -q tests/test_roadmap_118_live_safety_acceptance.py`, live-safety CLI smokes, Vite build, Dashboard V2 smoke/static verify, Playwright static browser smoke voor `/live`, `python -m binance_spot_bot.cli security-scan`, `python -m binance_spot_bot.check_all --skip-tests`, en volledige `pytest -q`.

Resultaat volledige test-suite: 455 passed, 1 warning.

Safety: live blijft locked-by-default; normale runtime/launcher kunnen first-order execute niet aanroepen; first-order execute vereist exact confirm en fake-adapter pad; output blijft redacted en `live_trading_enabled=False`.
