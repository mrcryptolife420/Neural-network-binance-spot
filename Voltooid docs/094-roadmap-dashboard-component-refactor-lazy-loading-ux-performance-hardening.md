...
Gebouwd: dashboard registry, stable components, smoke v2, UX evidence. Dashboard surface en docs toegevoegd waar van toepassing.
...
Gebouwd:
- page registry metadata met lazy module targets, smoke-required flags en page performance budgets;
- dedicated lazy page module boundaries voor overview, demo spot trading, demo pilot en performance;
- backward-compatible `page_key()` contract voor bestaande dashboard architecture tests;
- dashboard component payload limits voor tabellen en debug JSON;
- redacted/limited debug rendering;
- echte `dashboard_smoke_v2()` i.p.v. facade;
- dashboard refactor safety contract, page registry contract, payload-limit docs en browser-smoke fallback docs;
- Streamlit shell caption `DASHBOARD REFACTOR - NO LIVE TRADING`;
- Windows-safe Playwright fallback via asyncio subprocess probe.

Validatie:
- `python -m pytest -q` -> 350 passed, 1 warning;
- `python -m pytest tests/test_roadmap_094_dashboard_refactor_acceptance.py tests/test_roadmaps_089_096_full_surface.py::test_094_dashboard_smoke_v2_surface -q` -> groen;
...
Safety:
- live trading blijft disabled;
- `live` is niet selecteerbaar;
- page registry blokkeert live-enabled pages;
- geen signed/account/order endpoint toegevoegd;
- dashboard refactor dupliceert geen runtime business logic.
